...
class RefrigeratorCreate extends StatefulWidget {
  const RefrigeratorCreate({super.key});

  @override
  State<RefrigeratorCreate> createState() => _RefrigeratorCreateState();
}

class _RefrigeratorCreateState extends State<RefrigeratorCreate> {
  ...
  Future<void> fetchIngredients(String dishID) async {
    try {
      ...
      if (selectedProductIds.isNotEmpty &&
        ...
        // Если блюдо содержит хотя бы один из выбранных продуктов, добавляем его в filteredDishesID
        filteredDishesID.add(dishID);
      }
      // Если блюдо не содержит ни одного из выбранных продуктов, не добавляем его в filteredDishesID
      return;
    } catch (e) {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text('Укажите продукты из вашего холодильника',
                style: Theme.of(context).textTheme.bodyMedium),
            SizedBox(height: MediaQuery.of(context).size.height * 0.01),
            Text('А мы подберем блюда с этими продуктами!',
                style: Theme.of(context).textTheme.bodyMedium),
            SizedBox(height: MediaQuery.of(context).size.height * 0.01),
                ...
                onPressed: () async {
                  ...
                  if (filteredDishesID.isNotEmpty) {
                    Navigator.of(context)
                        .pushNamed('/refrigeratorMenu', arguments: {
                      'filteredDishesID': filteredDishesID,
                      'menuCreated': filteredDishesID.isNotEmpty,
                    });
                  } else {
                    Navigator.of(context)
                        .pushNamed('/refrigeratorMenu', arguments: {
